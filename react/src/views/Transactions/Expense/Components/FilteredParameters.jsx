  ...
  return (
      <div>
        {queries.sectorNames && (
          <>
            {queries.sectorNames.map((name, index) => {
              return (
                <Badge bg="secondary ml-1" key={`badge-sector-${index}`}>
                  {name}
                  <FontAwesomeIcon
                    icon={faClose}
                    className={"badge-cursor float-end ml-2"}
                    onClick={() =>
                      updateQuery("sector", name, queries.sectorIDS[index])
                    }
                  />
                </Badge>
              );
        ...
        {queries.reference && (
          <>
            {queries.reference.map((ref, index) => {
              return (
                <Badge bg="secondary ml-1" key={`badge-reference-${index}`}>
                  {ref}
                  <FontAwesomeIcon
                    icon={faClose}
                    className={"badge-cursor float-end ml-2"}
                    onClick={() =>
                      updateQuery("reference", ref, queries.reference[index])
                    }
                  />
                </Badge>
              );
        ...
        {queries.income_type && (
          <>
            {queries.income_type.map((type, index) => {
              return (
                <Badge bg="secondary ml-1" key={`badge-income-type-${index}`}>
                  {type}
                  <FontAwesomeIcon
                    icon={faClose}
                    className={"badge-cursor float-end ml-2"}
        ...
        {queries.check_for && (
          <>
            <Badge bg="secondary m-1">
              Online Check Status-{queries.check_for}
              <FontAwesomeIcon
                icon={faClose}
                className={"badge-cursor float-end ml-2"}
                onClick={() => updateQuery("check_for")}
              />
            </Badge>
          </>
        )}
        {queries.check_from && (
          <>
            <Badge bg="secondary m-1">
              {queries.check_for === "checkinout"
                ? "Check in from"
                : queries.check_for === "checkin"
                ? "Check in from"
                : "Check out from"}
              -{queries.check_from}
              <FontAwesomeIcon
                icon={faClose}
                className={"badge-cursor float-end ml-2"}
                onClick={() => updateQuery("check_from")}
              />
            </Badge>
          </>
        )}
