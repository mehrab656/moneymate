...
const ExpenseFilter = ({
  ...
  const {
    ...
    isError: hasCategoryFetchingError,
  ...
  const handelSectorIds = (e, sector) => {
    ...
    if (e.target.checked) {
      ...
      sectorNames = sectorNames.filter(
        (sectorName) => sectorName !== sector.label
      );
    }
    setQueryParams({
      ...queryParams,
  ...
  const handelCategoryIDS = (e, category) => {
    ...
    if (e.target.checked) {
      ...
      categoryNames = categoryNames.filter(
        (catName) => catName !== category.label
      );
    }
    setQueryParams({
      ...queryParams,
  ...
  const showCurrentPan = (currentTab) => {
    if (currentTab === "filter-report-by-sector") {
      return (
        <Tab.Pane eventKey={"filter-report-by-sector"}>
          ...
          <span
            className={"results"}
          >{`Found ${filteredSectors.length} Results`}</span>
    ...
    if (currentTab === "filter-report-by-categories") {
      return (
        <Tab.Pane eventKey={"filter-report-by-categories"}>
          ...
          <span
            className={"results"}
          >{`Found ${filteredCategories.length} Results`}</span>
          <div className={"report-filter-list"}>
          ...
          <div className="form-group">
            ...
            <Select
              ...
              value={
                ...
                ].find((opt) => opt.value === (queryParams.orderBy || "")) ||
                null
              }
              onChange={(opt) => {
            ...
            <Select
              ...
              value={
                [10, 20, 50, 100, 500, 1000]
                  ...
                  .find(
                    (opt) =>
                      opt.value === (String(queryParams.limit || "") || "")
                  ) || null
              }
              onChange={(opt) => {
                const value = opt?.value || "";
              ...
              options={[10, 20, 50, 100, 500, 1000].map((n) => ({
                value: String(n),
                label: String(n),
              }))}
    ...
    if (currentTab === "filter-report-by-date") {
      return (
        <Tab.Pane eventKey={"filter-report-by-date"}>
